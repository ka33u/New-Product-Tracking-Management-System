...
export function ProjectWorkspace({ project, snapshot, currentUser, onBack, onAction, onUpload }: { project: NpdProject; snapshot: NpdWorkspaceSnapshot; currentUser: NpdUser; onBack: () => void; onAction: RunAction; onUpload: UploadFile }) {
  ...
  return <div className="npd2-project-shell">
    ...
    {dialog?.kind === "part" && <PartDialog project={project} motors={motors} onClose={() => setDialog(null)} onAction={onAction} />}
...
function Overview({ project, snapshot, currentUser, motors, activities, steward, onDialog }: { project: NpdProject; snapshot: NpdWorkspaceSnapshot; currentUser: NpdUser; motors: ProjectMotor[]; activities: NpdWorkspaceSnapshot["activities"]; steward: boolean; onDialog: (dialog: DialogState) => void }) {
  ...
  return <div className="npd2-overview-grid"><div className="npd2-overview-main"><article className="npd2-panel"><div className="npd2-panel-title"><div><h2>同系列电机规格</h2><p>每个规格独立关联设计输出、检验要求、试验要求和完成节点</p></div>{steward && <button className="npd2-button npd2-button-soft" onClick={() => onDialog({ kind: "motor" })}><Icon name="plus" />增加规格</button>}</div><div className="npd2-motor-cards">{motors.map((motor) => <div className="npd2-motor-card" key={motor.id}><div className="npd2-motor-card-head"><span><Icon name="motor" /></span><div><b>{motor.model}</b><small>{motor.motorCode || "未设电机编码"}</small></div><StatusBadge value={motor.status} label={motor.status === "planned" ? "计划中" : motor.status} /></div><div className="npd2-motor-specs"><span><small>额定功率</small><b>{motor.ratedPower || "—"}</b></span><span><small>电压 / 频率</small><b>{motor.voltage || "—"} / {motor.frequency || "—"}</b></span><span><small>极数 / 机座</small><b>{motor.poles || "—"} / {motor.frameSize || "—"}</b></span><span><small>数量 / 节点</small><b>{motor.quantity} 台 / {motor.plannedDate}</b></span></div><div className="npd2-requirement"><small>检验要求</small><p>{motor.inspectionRequirement || "尚未配置"}</p><small>试验要求</small><p>{motor.testRequirement || "尚未配置"}</p></div>{(currentUser.role === "admin" || currentUser.role === "design" || steward) && <button className="npd2-text-button" onClick={() => onDialog({ kind: "motor", motor })}><Icon name="edit" />维护设计输出要求</button>}</div>)}</div></article><article className="npd2-panel"><div className="npd2-panel-title"><div><h2>项目交付完整度</h2><p>设计、生产、试验、质量交付物总览</p></div></div><div className="npd2-delivery-grid"><Delivery icon="folder" label="零部件节点" value={parts.filter((item) => item.status === "completed").length} total={parts.length} /><Delivery icon="file" label="试验报告" value={new Set(tests.map((item) => item.motorId)).size} total={motors.length} /><Delivery icon="shield" label="整机质量记录" value={new Set(inspections.filter((item) => item.itemType === "motor").map((item) => item.motorId)).size} total={motors.length} /><Delivery icon="check" label="零部件检验" value={new Set(inspections.filter((item) => item.itemType === "part").map((item) => item.partItemId)).size} total={parts.filter((item) => item.inspectionRequirement).length} /></div></article></div><aside className="npd2-overview-side"><article className="npd2-panel"><div className="npd2-panel-title"><div><h2>项目团队</h2><p>{members.length} 人协同</p></div>{steward && <button className="npd2-icon-button" onClick={() => onDialog({ kind: "member" })}><Icon name="plus" /></button>}</div><div className="npd2-member-list">{members.map((member) => <div key={member.id}><span>{member.userName.slice(-2)}</span><div><b>{member.userName}<em>{member.roleLabel}</em></b><p>{member.responsibility}</p></div></div>)}</div></article><article className="npd2-panel"><div className="npd2-panel-title"><div><h2>最近动态</h2><p>全操作时间戳</p></div></div><div className="npd2-timeline">{activities.slice(0, 12).map((activity) => <div key={activity.id}><i /><div><b>{activity.action}<span>{activity.actorName}</span></b><p>{activity.detail}</p><small>{formatDateTime(activity.createdAt)}</small></div></div>)}</div></article></aside></div>;
}
...
function PartsTable({ parts, currentUser, editable, onDialog }: { parts: NpdWorkspaceSnapshot["parts"]; currentUser: NpdUser; editable: boolean; onDialog: (dialog: DialogState) => void }) {
  ...
  return <article className="npd2-panel"><div className="npd2-panel-title"><div><h2>零部件明细与计划节点</h2><p>生产负责整机及零部件时间节点确认，检验要求绑定设计输出</p></div>{editable && <button className="npd2-button npd2-button-primary" onClick={() => onDialog({ kind: "part" })}><Icon name="plus" />新增零部件</button>}</div>{parts.length ? <div className="npd2-table-wrap"><table className="npd2-table"><thead><tr><th>零部件</th><th>关联规格</th><th>规格 / 材质</th><th>设计输出与检验要求</th><th>计划节点</th><th>生产状态</th><th>确认</th></tr></thead><tbody>{parts.map((part) => <tr key={part.id}><td><b>{part.partNo}</b><span>{part.name} · {part.sourceType} · {part.quantity}件</span></td><td>{part.motorModel || "系列通用"}</td><td>{part.specification || "—"}<span>{part.material || "—"}</span></td><td><small className="npd2-ref">{part.designOutputRef}</small><span>{part.inspectionRequirement}</span></td><td>{part.plannedDate}<span>{part.actualDate ? `实际 ${part.actualDate}` : "未完工"}</span></td><td><StatusBadge value={part.status} label={part.status === "completed" ? "已完成" : part.status === "blocked" ? "受阻" : part.status === "in_progress" ? "进行中" : "计划中"} />{part.confirmedByName && <span>{part.confirmedByName} · {formatDateTime(part.confirmedAt)}</span>}</td><td>{canConfirm ? <button className="npd2-icon-button" onClick={() => onDialog({ kind: "confirm_part", partId: part.id })}><Icon name="check" /></button> : <span>—</span>}</td></tr>)}</tbody></table></div> : <EmptyState title="尚无零部件节点" detail="新增零部件后，生产人员可确认进行、完成或受阻状态。" action={editable && <button className="npd2-button npd2-button-primary" onClick={() => onDialog({ kind: "part" })}>新增第一条记录</button>} />}</article>;
}
...
function Reports({ project, snapshot, editable, onDialog }: { project: NpdProject; snapshot: NpdWorkspaceSnapshot; editable: boolean; onDialog: (dialog: DialogState) => void }) {
  ...
  const motors = snapshot.motors.filter((item) => item.projectId === project.id);
  return <article className="npd2-panel"><div className="npd2-panel-title"><div><h2>规格级试验报告</h2><p>每个电机规格至少需一份有效试验报告方可完成验证阶段</p></div>{editable && <button className="npd2-button npd2-button-primary" onClick={() => onDialog({ kind: "test" })}><Icon name="plus" />提交试验报告</button>}</div><div className="npd2-report-grid">{motors.map((motor) => { const items = reports.filter((item) => item.motorId === motor.id); return <div className="npd2-report-group" key={motor.id}><header><span><Icon name="motor" /></span><div><b>{motor.model}</b><small>{items.length} 份报告</small></div>{items.length > 0 ? <StatusBadge value={items.some((item) => item.result === "不合格") ? "blocked" : "completed"} label={items.some((item) => item.result === "不合格") ? "存在不合格" : "已覆盖"} /> : <StatusBadge value="not_started" label="缺少报告" />}</header>{items.map((item) => <div className="npd2-report-item" key={item.id}><div><b>{item.reportNo} · {item.title}</b><span>{item.reportType} · {item.testDate} · {item.submittedByName}</span><p>{item.conclusion || item.requirementRef}</p></div><StatusBadge value={item.result === "不合格" ? "blocked" : "completed"} label={item.result} />{item.documentId && <a href={`/api/files/${item.documentId}`} title={item.fileName}><Icon name="file" /></a>}</div>)}</div>; })}</div></article>;
}
...
function Inspections({ project, snapshot, editable, onDialog }: { project: NpdProject; snapshot: NpdWorkspaceSnapshot; editable: boolean; onDialog: (dialog: DialogState) => void }) {
  const inspections = snapshot.inspections.filter((item) => item.projectId === project.id);
  return <article className="npd2-panel"><div className="npd2-panel-title"><div><h2>质量检验记录</h2><p>整机与零部件检验依据由设计输出自动带入，记录不可脱离要求创建</p></div>{editable && <button className="npd2-button npd2-button-primary" onClick={() => onDialog({ kind: "inspection" })}><Icon name="plus" />提交检验记录</button>}</div>{inspections.length ? <div className="npd2-inspection-list">{inspections.map((item) => <div key={item.id}><span className={`npd2-inspection-icon ${item.itemType}`}><Icon name={item.itemType === "motor" ? "motor" : "folder"} /></span><div className="npd2-inspection-main"><div><b>{item.itemName}</b><small>{item.itemType === "motor" ? "整机检验" : "零部件检验"} · {item.inspectionDate}</small></div><p><em>设计要求</em>{item.inspectionRequirement}</p><span>{item.conclusion}</span></div><div className="npd2-inspection-result"><StatusBadge value={item.result === "不合格" ? "blocked" : "completed"} label={item.result} /><small>{item.inspectorName}<br />{formatDateTime(item.createdAt)}</small></div>{item.documentId && <a className="npd2-icon-button" href={`/api/files/${item.documentId}`}><Icon name="file" /></a>}</div>)}</div> : <EmptyState icon="shield" title="尚无质量检验记录" detail="质量人员可选择整机或零部件，系统会从设计输出自动引用检验要求。" />}</article>;
}
