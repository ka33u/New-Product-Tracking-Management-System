...
export function CreateProjectDialog({ snapshot, onClose, onAction }: { snapshot: NpdWorkspaceSnapshot; onClose: () => void; onAction: RunAction }) {
  ...
  const blankMotor = () => ({ id: crypto.randomUUID(), model: "", motorCode: "", ratedPower: "", voltage: "380V", frequency: "50Hz", poles: "", speed: "", frameSize: "", mounting: "", quantity: 1, inspectionRequirement: "", testRequirement: "", plannedDate: addDays(90) });
...
export function MotorDialog({ project, motor, onClose, onAction }: { project: NpdProject; motor?: ProjectMotor; onClose: () => void; onAction: RunAction }) {
  const [busy, setBusy] = useState(false);
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); try { const raw = getFormObject(event.currentTarget); if (motor) await onAction("update_motor_requirements", { motorId: motor.id, inspectionRequirement: raw.inspectionRequirement, testRequirement: raw.testRequirement }); else await onAction("add_motor", { projectId: project.id, motor: { ...raw, quantity: Number(raw.quantity || 1) } }); onClose(); } finally { setBusy(false); } };
  return <Modal title={motor ? `维护 ${motor.model} 设计要求` : "增加电机规格"} eyebrow={project.code} onClose={onClose} wide><form onSubmit={submit}><div className="npd2-form-grid">{!motor && <><Field label="型号规格" required><input name="model" required /></Field><Field label="电机编码"><input name="motorCode" /></Field><Field label="额定功率"><input name="ratedPower" /></Field><Field label="电压"><input name="voltage" defaultValue="380V" /></Field><Field label="频率"><input name="frequency" defaultValue="50Hz" /></Field><Field label="极数"><input name="poles" /></Field><Field label="转速"><input name="speed" /></Field><Field label="机座号"><input name="frameSize" /></Field><Field label="安装方式"><input name="mounting" /></Field><Field label="数量"><input name="quantity" type="number" min="1" defaultValue="1" /></Field><Field label="计划完成" required><input name="plannedDate" type="date" required defaultValue={project.plannedEnd} /></Field></>}<Field label="检验要求" required full hint="后续质量检验记录将自动引用此处，不能在检验阶段自行篡改。"><textarea name="inspectionRequirement" required rows={4} defaultValue={motor?.inspectionRequirement} /></Field><Field label="试验要求" required full hint="后续试验报告必须关联该设计输出要求。"><textarea name="testRequirement" required rows={4} defaultValue={motor?.testRequirement} /></Field></div><SubmitBar busy={busy} onCancel={onClose} /></form></Modal>;
}
...
export function SheetDialog({ project, sheet, onClose, onAction }: { project: NpdProject; sheet: ProjectSheet; onClose: () => void; onAction: RunAction }) {
  const [busy, setBusy] = useState(false);
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); try { const raw = getFormObject(event.currentTarget); await onAction("update_sheet", { projectId: project.id, sheetCode: sheet.code, status: raw.status, progress: Number(raw.progress), plannedDate: raw.plannedDate, note: raw.note }); onClose(); } finally { setBusy(false); } };
  return <Modal title={`更新 ${sheetByCode[sheet.code].shortTitle}`} eyebrow="阶段状态与放行" onClose={onClose}><form onSubmit={submit}><div className="npd2-gate-note"><Icon name="shield" /><p>阶段标记“已完成”时，系统会校验前置阶段、受控表单、零部件节点、试验报告和质量检验的齐套性。</p></div><div className="npd2-form-grid npd2-form-grid-one"><Field label="阶段状态" required><select name="status" defaultValue={sheet.status}>{Object.entries(sheetStatusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label="完成度" required><input name="progress" type="number" min="0" max="100" defaultValue={sheet.progress} /></Field><Field label="计划完成日期" required><input name="plannedDate" type="date" required defaultValue={sheet.plannedDate} /></Field><Field label="说明/受阻原因"><textarea name="note" rows={4} defaultValue={sheet.note} /></Field></div><SubmitBar busy={busy} onCancel={onClose} primary="更新阶段" /></form></Modal>;
}

export function PartDialog({ project, motors, onClose, onAction }: { project: NpdProject; motors: ProjectMotor[]; onClose: () => void; onAction: RunAction }) {
  const [busy, setBusy] = useState(false);
  const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setBusy(true); try { const raw = getFormObject(event.currentTarget); await onAction("add_part", { ...raw, projectId: project.id, motorId: raw.motorId || null, quantity: Number(raw.quantity || 1) }); onClose(); } finally { setBusy(false); } };
  return <Modal title="新增零部件与计划节点" eyebrow={`${project.code} · Sheet 5`} onClose={onClose} wide><form onSubmit={submit}><div className="npd2-form-grid"><Field label="关联电机规格"><select name="motorId" defaultValue=""><option value="">系列通用件</option>{motors.map((motor) => <option key={motor.id} value={motor.id}>{motor.model}</option>)}</select></Field><Field label="零部件编号" required><input name="partNo" required /></Field><Field label="零部件名称" required><input name="name" required /></Field><Field label="规格"><input name="specification" /></Field><Field label="材质"><input name="material" /></Field><Field label="数量"><input name="quantity" type="number" min="1" defaultValue="1" /></Field><Field label="来源"><select name="sourceType" defaultValue="自制"><option>自制</option><option>外协</option><option>外购</option><option>借用</option></select></Field><Field label="计划完成" required><input name="plannedDate" type="date" required defaultValue={project.plannedEnd} /></Field><Field label="设计输出引用" required full hint="例如：图纸 HD-YBX5-001 / BOM A1 第 12 项"><input name="designOutputRef" required /></Field><Field label="检验要求" required full><textarea name="inspectionRequirement" required rows={3} placeholder="尺寸、材质、性能、抽检比例及合格判据" /></Field><Field label="试验要求" full><textarea name="testRequirement" rows={3} placeholder="如该零部件需专项试验，请填写项目及判定要求" /></Field></div><SubmitBar busy={busy} onCancel={onClose} primary="加入节点计划" /></form></Modal>;
}
